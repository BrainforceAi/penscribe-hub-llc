// Layout.jsx — Nav, Footer, BookingModal, hash router. Globals on window.
...
// ===== Booking modal — global open/close via event bus =====
function BookingModal() {
  ...
  if (!open) return null;
  const body = `Hi Isaac,%0D%0A%0D%0AI would like to discuss${subject ? ` "${subject}"` : ' a government contracting opportunity'}.%0D%0A%0D%0AAbout my business:%0D%0A%0D%0A%0D%0ARelevant solicitation (if any):%0D%0A%0D%0A%0D%0AThank you,%0D%0A`;
  const mailto = `mailto:isaac@thepenscribehub.com?subject=${encodeURIComponent('Consultation request' + (subject ? ' — ' + subject : ''))}&body=${body}`;
  return (
    <div className="modal-overlay" onClick={() => setOpen(false)}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="eyebrow">Discovery call</div>
        <h3 style={{marginTop: 8}}>Tell me about your opportunity.</h3>
        <p>Send me a short note with your business, the solicitation (if you have one), and your timeline. I will reply within one business day. There is no charge for the first review.</p>
        <div style={{marginTop: 24, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap'}}>
          <a className="btn btn-primary" href={mailto}>Open my email <span>→</span></a>
          <a className="btn btn-secondary" href="#/contact" onClick={() => setOpen(false)}>Use the form instead</a>
        </div>
    ...
    <nav className={`nav ${scrolled ? 'scrolled' : ''}`}>
      <div className="nav-inner">
        <a href="#/" className="nav-mark">
          <img src="../assets/penscribe-logo-on-white.jpg" alt="The PenScribe Hub" />
          <div className="word">
        ...
        <div className="nav-links">
          ...
          <NavLink href="#/contracts" current={hash}>Contracts</NavLink>
          <NavLink href="#/contact" current={hash}>Contact</NavLink>
          <button className="btn btn-primary" style={{padding: '10px 18px', fontSize: 14}} onClick={() => openBooking()}>
        ...
        <div className="footer-grid">
          <div>
            <div className="footer-mark">
              <img src="../assets/penscribe-logo-on-blue.jpg" alt="The PenScribe Hub" />
          ...
          <div>
            ...
            <ul>
              ...
              <li><a href="mailto:isaac@thepenscribehub.com">isaac@thepenscribehub.com</a></li>
            </ul>
          </div>
          <div>
            <h5>Credentials</h5>
